    ...
    <input type="button" value="Continue Shopping" onclick="window.location='index.php'" />
    	...
    	<?
			if(is_array($_SESSION['cart'])){
            	echo '<tr bgcolor="#FFFFFF" style="font-weight:bold"><td>ID</td><td>Name</td><td>Price</td><td align="center">Qty</td><td>Amount</td><td>Options</td></tr>';
				$max=count($_SESSION['cart']);
				for($i=0;$i<$max;$i++){
					$pid=$_SESSION['cart'][$i]['productid'];
			...
			?>
                    ...
                    <td align="right">$ <?=number_format(get_price($pid), 2)?></td>
                    <td align="center"><input type="text" name="product<?=$pid?>" value="<?=$q?>" maxlength="3" size="2" align="center"/></td>                    
                    <td align="right">$ <?=number_format (get_price($pid)*$q, 2)?></td>
